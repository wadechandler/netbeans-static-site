...
/*
function download(option) {
    // show survey form and store selected value for later
    $('#selopt').val(option);
    var cookie=get_cookie('nbsurvey71');
    var url=setupUrl();
    //alert(url);
    if(!cookie) {
		set_cookie ( 'nbsurvey71', 1, 2012, 12, 31, '/' ); 
		$.colorbox({width:"650px", height:"780px", close:"No Thanks. Just take me to the download", iframe:true, href:"http://services.netbeans.org/appl/index.php?mod=dl_survey&sec=form&dom=m&url=downloads/"+url,opacity:"0.6", onCleanup:function(){redir();}});		
	} else {
		redir();
	}
}
